...
class RNNModel(nn.Module):
    """
    Multi-horizon SimpleRNN with a shared backbone and linear output head.

    Args:
        in_dim  : number of input features
        hidden  : hidden size of RNN
...
def train_one_model(tr_dl, q, label, epochs=EPOCHS, patience=PATIENCE):
    """
    Train a single quantile model with early stopping.
...
for win in WINDOW_SIZES:
    ...
    print(f"  Training Upper model (q={Q_UPPER}) ...")
    m_up, h_up = train_one_model(tr_dl, Q_UPPER, "UP")

    print(f"  Training Lower model (q={Q_LOWER}) ...")
...
# Re-train best models on full training data (already done above — reuse)
tr_dl_b, _, _ = get_loaders(best_win)
print(f"\nRe-using trained models for window={best_win} for 5-day forecast ...")

# Load best upper/lower models from results (already trained)
...
fig1.suptitle(
    f"Nifty50 % Change — SimpleRNN  |  Two Quantile Models  |  Best Window = {best_win}\n"
...
fig2.suptitle(
    f"Nifty50 SimpleRNN — Grid Search Comparison  |  Windows {WINDOW_SIZES}\n"
